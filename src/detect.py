...
def detect_objects_and_save_separately(las_file_path, a, b, d, threshold_reflectance, eps, min_samples,
    ...
    for label in unique_labels:
        ...
        if (np.max(cluster[:, 0]) - np.min(cluster[:, 0]) < 0.5 and  
            np.max(cluster[:, 1]) - np.min(cluster[:, 1]) < 0.5 and  
            np.max(cluster[:, 2]) - np.min(cluster[:, 2]) < 0.5):  
            ...
            print(f"Cluster {label} 超出边长为0.5的正方体约束")
...
if __name__ == '__main__':
    ...
    detect_objects_and_save_separately(las_file_path=r"data\pp_data\direction0-2cm-40m_pp.las", a=C[0], b=C[1], d=C[2], eps=0.2,
